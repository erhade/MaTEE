 ...
 */
#include <include/heap_param_ta.h>
#include <string.h>
...
{
	switch (cmd_id) {
	case TA_HEAP_PARAM_CMD_ALLOC_HEAP:
		return alloc_heap(param_types, params);

	case TA_HEAP_PARAM_CMD_READ_HEAP:
		return read_heap(param_types, params);

	case TA_HEAP_PARAM_CMD_WRITE_HEAP:
		return write_heap(param_types, params);

	case TA_HEAP_PARAM_CMD_RELEASE_HEAP:
